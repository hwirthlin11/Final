...
if (firstName === "" || lastName === "" || username === "" || password === "") {
  console.log("Please enter a first name, last name, username, and password");
  process.exit();
}
...
User.findOne({
  ...
  if (user) {
    console.log("That username already exists! Try again...");
    process.exit();
  }
}).then(() => {
  let user = new User({
  ...
  user.save().then(() => {
    console.log("Admin user created for", firstName, lastName, ", username: ", username);
    process.exit();
  });
}).catch(error => {
  console.log(error);
